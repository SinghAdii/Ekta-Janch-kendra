...
export default function OnlinePackageBookingPage() {
  // React Query for data fetching with caching
  const { data: orders = [], isLoading, refetch, isFetching } = useOrdersBySource("Online Package Booking");
  ...
  const {
    ...
    clearFilters,
    hasActiveFilters,
  } = useOrderFilters(orders, "Online Package Booking");
      ...
      <div className="p-6 space-y-6 max-w-full mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Online Package Bookings</h1>
              <p className="text-sm text-muted-foreground">Health packages booked through website/app</p>
            </div>
          </div>
          <Button
            variant="outline"
        ...
        <Card>
          <CardContent className="p-4">
            <OrderFiltersBar
              ...
              searchPlaceholder="Search by order #, patient, or package..."
            />
          </CardContent>
        </Card>
          ...
          {isLoading ? (
            ...
            <Card className="col-span-full">
              <CardContent className="p-12 text-center text-muted-foreground">
                No online package bookings found
              </CardContent>
            </Card>
          ) : (
            filteredOrders.map((order, i) => (
              ...
              >
                <Card className="hover:shadow-lg transition-shadow overflow-hidden border-l-4 border-l-purple-500">
                      ...
                      {order.packages.map((pkg) => (
                        <div key={pkg.id} className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground">
                            ({pkg.testsIncluded.length} tests)
                          </span>
                        </div>
                      ))}
                    </div>
